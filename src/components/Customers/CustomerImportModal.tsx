...
export default function CustomerImportModal({ onClose, onSuccess }: CustomerImportModalProps) {
  ...
  const downloadLogs = () => {
    const headers = [['Baris', 'Nama Customer', 'Status', 'Pesan', 'Waktu']];
    ...
    XLSX.writeFile(wb, `Import_Log_Customer_${new Date().toISOString().split('T')[0]}.xlsx`);
  };
  ...
  const upsertWithRetry = async (payload: Record<string, unknown>, retries = 0): Promise<{ error: any }> => {
    try {
      const { error } = await supabase
        .from('customers')
        .upsert(payload, { onConflict: 'id' }); // Changed from 'email' to 'id' (PK) as a temporary fix, or use a unique column if it exists
  ...
  const confirmImport = async () => {
    ...
    try {
      ...
      await logImportActivity('customers', validData.length, processedCount, errorCount, profile?.id);
  ...
  const downloadTemplate = () => {
    const headers = [
      ['Nama Customer', 'Email', 'Telepon', 'Alamat', 'Limit Kredit', 'Termin Pembayaran'],
      ['PT Contoh Sukses', 'info@contoh.com', '08123456789', 'Jl. Merdeka No. 10', 10000000, 'Net 30']
    ];
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (evt) => {
      try {
        ...
        const processedData: CustomerImportData[] = rows.map((row) => ({
          name: String(row[0] || ''),
          email: String(row[1] || ''),
          phone: String(row[2] || ''),
          address: String(row[3] || ''),
          credit_limit: Number(row[4] || 0),
          payment_terms: String(row[5] || ''),
        ...
        const validatedData = processedData.map(item => {
          ...
          if (isNaN(item.credit_limit)) errorMsg += 'Limit kredit harus angka. ';
            ...
            <div className="space-y-6">
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 text-sm text-blue-700">
                ...
                <ul className="list-disc ml-5 space-y-1">
                  ...
                  <li>Kolom Nama Customer wajib diisi.</li>
                  <li>Jika Email sudah ada, data customer akan diperbarui.</li>
                </ul>
              </div>
